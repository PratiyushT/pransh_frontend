<script lang="ts">
  ...
  // For marking fields as touched (for edit intent)
  ...
  const toggleCurrentPasswordVisibility = () => showCurrentPassword = !showCurrentPassword;
  ...
  const handleProfileUpdate = async () => {
    try {
      ...
      // Step 2: Fetch the latest profile data to get the original phone number
      let userData = null;
      try {
        const { data: userDataResp, error: userDataError } = await supabase
          .from('profiles')
          .select('*')
        ...
        // If we can't fetch, fallback to empty string for originalPhoneNumber
        userData = {};
      }
      ...
      const originalPhoneNumber = userData?.phone_number || '';
      console.log('Original phone:', originalPhoneNumber, 'Current phone:', user.phone_number);

      const isPhoneChanged = user.phone_number !== originalPhoneNumber;
      ...
      if (hasUpdates && !profileUpdateMessage) {
        ...
        // Always clear current password after a successful save
        currentPassword = '';
      } else if (!hasUpdates) {
      ...
      profileUpdateError = true;
      // Do NOT clear currentPassword here; only clear on successful save
    } finally {
      isUpdatingProfile = false;
    }
  };
  ...
  const checkAuth = async () => {
    try {
      ...
      if (userError) {
        ...
        // Set originalPhoneNumber to empty string if we can't fetch
        originalPhoneNumber = '';
      } else if (userData) {
        ...
        // Properly set phone number - check multiple fields and use correct field
        // The phone field in the database might have different names
        user.phone_number = userData.phone_number || userData.phone || '';

        // Store the original phone number for change detection
        originalPhoneNumber = userData.phone_number || userData.phone || '';

        // Ensure the reactive binding by also setting these values directly on form elements
        // This addresses potential reactivity issues in Svelte
        setTimeout(() => {
          const phoneValue = user.phone_number || '';
          console.log('Setting phone value:', phoneValue);

          document.getElementById('settings-firstname')?.setAttribute('value', user.firstName || '');
          document.getElementById('settings-lastname')?.setAttribute('value', user.lastName || '');
          document.getElementById('settings-email')?.setAttribute('value', user.email || '');
          document.getElementById('settings-phone')?.setAttribute('value', phoneValue);

          // Also try using value property as a fallback
  ...
  const handleAddressSaved = async (event) => {
    const { savedAddress, allAddresses, action } = event.detail;

    console.log(`Address ${action}d successfully:`, savedAddress);

    // Immediately update UI with the new addresses if they were returned
    if (allAddresses && Array.isArray(allAddresses)) {
      // Sort addresses to ensure default address is first
      user.addresses = allAddresses.sort((a, b) => {
        // Default address should be first (prioritize is_default flag)
        if (a.is_default && !b.is_default) return -1;
        if (!a.is_default && b.is_default) return 1;
        // Otherwise maintain original order
      ...
      // Fallback to fetching addresses if they weren't returned
  ...
  onMount(async () => {
    ...
    try {
      ...
      // This ensures the form is pre-filled with the user's current information
      if (user) {
        // Update account settings section with current user data
        document.getElementById('settings-firstname')?.setAttribute('value', user.firstName || '');
        document.getElementById('settings-lastname')?.setAttribute('value', user.lastName || '');
        document.getElementById('settings-email')?.setAttribute('value', user.email || '');
        document.getElementById('settings-phone')?.setAttribute('value', user.phone_number || '');

        // Mark fields as touched so validation styles are applied
        firstNameTouched = !!user.firstName;
        lastNameTouched = !!user.lastName;
        emailTouched = !!user.email;
        phoneTouched = !!user.phone_number;
      }
