  ...
  <div class="address-fields space-y-6 {showAddressToggle ? 'border-t border-b border-gray-200 py-6 mt-4 animate-fade-in' : ''}">
    ...
    {#if showManualAddressForm || addressSelected || !showMapboxSearch}
      <div class="form-field animate-fade-in">
        ...
        {/if}
        <label for="streetAddress" class="block text-sm font-medium text-gray-700 mb-1">Street Address {#if addressRequired}<span class="text-red-500">*</span>{/if}</label>
        <input
          id="streetAddress"
          type="text"
          bind:value={addressData.street}
          on:input={() => { resetFieldError(); emitChange(); }}
          on:blur={() => { streetAddressTouched = true; addressLine1Touched = true; emitChange(); dispatch('touchField', 'addressLine1'); }}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
        <div class="form-field">
          <label for="city" class="block text-sm font-medium text-gray-700 mb-1">City {#if addressRequired}<span class="text-red-500">*</span>{/if}</label>
          <input
            id="city"
            type="text"
            bind:value={addressData.city}
            on:input={() => { resetFieldError(); emitChange(); }}
            on:blur={() => { cityTouched = true; emitChange(); dispatch('touchField', 'city'); }}
            readonly={addressFromMapbox}
            required={addressRequired}
        ...
        <div class="form-field">
          <label for="state" class="block text-sm font-medium text-gray-700 mb-1">State/Province {#if addressRequired}<span class="text-red-500">*</span>{/if}</label>
          <input
            id="state"
            type="text"
            bind:value={addressData.state}
            on:input={() => { resetFieldError(); emitChange(); }}
            on:blur={() => { stateTouched = true; emitChange(); dispatch('touchField', 'state'); }}
            readonly={addressFromMapbox}
            required={addressRequired}
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 animate-fade-in">
        <div class="form-field">
          <label for="zipCode" class="block text-sm font-medium text-gray-700 mb-1">Zip/Postal Code {#if addressRequired}<span class="text-red-500">*</span>{/if}</label>
          <input
            id="zipCode"
            type="text"
            bind:value={addressData.postalCode}
            on:input={() => { resetFieldError(); emitChange(); }}
            on:blur={() => { zipCodeTouched = true; postalCodeTouched = true; emitChange(); dispatch('touchField', 'postalCode'); }}
            readonly={addressFromMapbox}
            required={addressRequired}
        ...
        <div class="form-field">
          <label for="country" class="block text-sm font-medium text-gray-700 mb-1">Country {#if addressRequired}<span class="text-red-500">*</span>{/if}</label>
          <select
            id="country"
      ...
      {#if showPhoneNumber}
        <div class="form-field animate-fade-in">
          <label for="shippingPhoneNumber" class="block text-sm font-medium text-gray-700 mb-1">
            Phone Number {#if phoneNumberRequired}<span class="text-red-500">*</span>{:else}<span class="text-gray-400 text-xs">(Optional)</span>{/if}
          </label>
          <input
            ...
            class="w-full px-3 py-2 border {streetAddressTouched && (!addressData.phoneNumber || !isPhoneNumberValid) && showAddressFields && phoneNumberRequired ? 'border-red-300' : 'border-gray-300'} focus:border-gold focus:ring focus:ring-gold/20 outline-none transition rounded-sm"
            placeholder="(555) 555-5555"
          >
          {#if streetAddressTouched && (!addressData.phoneNumber || !isPhoneNumberValid) && showAddressFields && phoneNumberRequired}
